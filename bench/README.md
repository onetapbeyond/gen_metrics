...
Now lets compare the results for the `untraced-pipeline [max_demand: 1]` and `traced_pipeline [max_demand: 1]` tests. In this case we see a significant impact on runtime performance, reported as approximately `2.06x slower`. In these tests because `max_demand` was constrained to a value of one GenStage could not perform any batching of events to statisfy upstream demand. Instead, every message was sent upstream one event at a time. So while the rate-of-throughput stayed the same, the rate-of-calls increased from 134 calls-per-second to 67k calls-per-second per stage in the pipeline. It is this massive jump in the rate-of-calls that explains the runtime overhead associated with the extremely busy GenMetrics monitoring agent.
