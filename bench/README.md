...
The answer is simple as soon as you understand the difference between the between the `rate-of-throughput` and the `rate-of-calls` within a pipeline.
...
Thanks to GenStage intelligently batching events to satisfy upstream demand the `rate-of-throughput` is high while the `rate-of-calls` to deliver that throughput is low. Specifically, in this test where `max_demand` was set of 1000, the rate-of-throughput was 67k messages-per-second while the rate-of-calls per stage in the pipeline was just 134 calls-per-second (67k / 500 = 134).

Now lets compare the results for the `untraced-pipeline [max_demand: 1]` and `traced_pipeline [max_demand: 1]` tests. In this case we see a significant impact on runtime performance, reported as approximately `2.06x slower`. In these tests because `max_demand` was constrained to a value of one GenStage could not perform any batching of events to statisfy upstream demand. Instead, every single message was sent upstream one event at a time. So while the rate-of-throughput stayed the same, the rate-of-calls increased from 134 calls-per-second to 67k calls-per-second per stage in the pipeline. It is this massive jump in the rate-of-calls that explains the significant runtime overhead caused by an extremely busy GenMetrics monitoring agent.
