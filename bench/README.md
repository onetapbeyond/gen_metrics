...
Keep in mind, on our test hardware the GenMetrics tracing agent monitored 2.0 million synchronous calls in 30 seconds. This is a good example of what we described as `a high rate of synchronous calls` in the [GenMetrics + Synchronous Calls](#genmetrics--synchronous-calls) section.
...
At what exact call rate the impact of monitoring by GenMetrics becomes problematic is going to be entirely dependent on the nature of the message passing to your server or through your pipeline and on the hardware resources on which your application is running.
