...
To activate sampling metrics for your cluster or pipeline simply specify the `sample_rate` option when declaring the monitoring preferences. For example, to reduce the runtime overhead of GenMetrics by sampling just 10% of all callbacks simply specify `opts : [sample_rate: 0.1]`.
