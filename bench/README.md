...
First lets compare the results for `untraced-pipeline [max_demand: 1000]` and `traced-pipeline [max_demand: 1000]`. The `max_demand` value indicated is in fact the default value for a GenStage pipeline. Notice how there is very low runtime overhead on the `traced-pipeline [max_demand: 1000]`, just `1.07 times` slower than the untraced pipeline. Considering the test push 2 million messages (or 67k messages/s) this result indicates the GenMetrics runtime impact was neglible. Thanks to GenStage intelligently batching events to satisfy upstream demand the `rate-of-throughput` is high while the `rate-of-calls` to deliver that throughput is low. Specifically, in this test where `max_demand` was 1000, the rate-of-throughput was 67k messages per second while the rate-of-calls was just 134 calls per second.
