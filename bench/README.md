...
Thanks to GenStage intelligently batching events to satisfy upstream demand the `rate-of-throughput` for events is high while the `rate-of-calls` to deliver that throughput is low. Specifically, in this test where `max_demand` was set to 1000, the rate-of-throughput was 67k messages-per-second while the rate-of-calls-per-stage in the pipeline was just 134 calls-per-second (67k / 500 = 134).
...
> GenMetrics can safely monitor a GenStage pipeline with a high rate-of-throughput as long as the rate-of-calls within that pipeline is low or moderate. However, above a certain rate-of-calls within the pipeline, the runtime overhead of GenMetrics will impact negatively on your application's performance.
