...
data = Enum.map(1..10_000, fn i -> %{id: i, data: String.duplicate("a", 100)} end)
...
Benchee.run(%{time: 30, warmup: 5}, %{
      "untraced-server [ call ]" => fn ->
          ...
          for item <- data do
            ...
            # Standardize message flow rate across tests.
            Process.sleep(1)
          end
          receive do
      ...
      "untraced-server [ cast ]" => fn ->
        UntracedServer.init_state(length(data))
        pid = self()
        for item <- data do
          UntracedServer.do_cast(%{item | id: pid})
          # Standardize message flow rate across tests.
          Process.sleep(1)
        end
        receive do
      ...
      "traced---server [ cast ]" => fn ->
        TracedServer.init_state(length(data))
        pid = self()
        for item <- data do
          TracedServer.do_cast(%{item | id: pid})
          # Standardize message flow rate across tests.
          Process.sleep(1)
        end
        receive do
      ...
      "untraced-server [ info ]" => fn ->
        UntracedServer.init_state(length(data))
        pid = self()
        for item <- data do
          UntracedServer.do_info(%{item | id: pid})
          # Standardize message flow rate across tests.
          Process.sleep(1)
        end
        receive do
      ...
      "traced---server [ info ]" => fn ->
        TracedServer.init_state(length(data))
        pid = self()
        for item <- data do
          TracedServer.do_info(%{item | id: pid})
          # Standardize message flow rate across tests.
          Process.sleep(1)
        end
        receive do
